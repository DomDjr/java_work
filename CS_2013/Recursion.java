...
/**
 * Solutions to L03: Recursion practice.
 */

public class Recursion {

	// add decimal ASCII values of all characters in a String
	public static int sumASCII(String s) {
		if (s == null || s.isEmpty())
			return 0;
		else
			return sumASCII(s, 0, 0);
	}

	private static int sumASCII(String s, int index, int accumulator) {
		if (index == s.length()) {
			accumulator++;
		}
			return sumASCII(s, index+1 ,s.charAt(index) + accumulator);
	}
